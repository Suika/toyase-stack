...
MYSQL_PWD=${DB_SPHINX_PASS}
export MAIN_TABLE=$(echo "SHOW TABLES;" | mysql --host=${DB_HOST} --user=${DB_SPHINX_USER} --password=${DB_SPHINX_PASS} --silent --skip-column-names ${DB_ASAGI_NAME} | grep -vE "(daily|deleted|images|threads|users|index)" | head -n 1)
cat << EOF
###########################################
...
source main
{
  ...
  sql_query      = \\
      SELECT doc_id, "${MAIN_TABLE}" AS board, timestamp AS timestamped, thread_num AS tnum, num, subnum, name, trip, email, title, comment, \\
...
for BOARD in $(echo "SHOW TABLES;" | mysql --host=${DB_HOST} --user=${DB_SPHINX_USER} --password=${DB_SPHINX_PASS} --silent --skip-column-names ${DB_ASAGI_NAME} | grep -vE "(daily|deleted|images|threads|users|index)" ); do
cat << EOF
# /${BOARD}/
